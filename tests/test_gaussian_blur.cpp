            ...
            for (int channel = 0; channel < channels; ++channel) {
                ...
                for (int ky = -radius; ky <= radius; ++ky) {
                    ...
                    for (int kx = -radius; kx <= radius; ++kx) {
                        ...
                        sum += weight * static_cast<unsigned int>(src[static_cast<std::size_t>((src_y * width + src_x) * channels + channel)]);
                    }
                }

                dst[static_cast<std::size_t>((y * width + x) * channels + channel)] = static_cast<std::uint8_t>((sum + (weight_sum / 2u)) / weight_sum);
            }
        }
    }
    ...
    int kernel_size)
{
    ...
    const auto expected = gaussian_blur_reference(input, shape.width, shape.height, shape.channels, kernel_size);
    if (output != expected) {
...
int main()
{
    ...
    expect(run_case(gray, hipcv::PixelFormat::gray8, 5, 4, 1, 3), "3x3 gray gaussianBlur should match reference");
    expect(run_case(gray, hipcv::PixelFormat::gray8, 5, 4, 1, 5), "5x5 gray gaussianBlur should match reference");
    expect(run_case(bgr, hipcv::PixelFormat::bgr8, 3, 3, 3, 3), "3x3 BGR gaussianBlur should match reference");
